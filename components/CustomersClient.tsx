...
function ContactDetailSheet({ contact, onClose }: { contact: Lead; onClose: () => void }) {
  ...
  const sourceLabel = c.source === "quiz_funnel" ? "Quiz funnel" : c.source === "contact_form" ? "Contact form" : c.source === "manual" ? "Manual" : c.source;
  const tags = c.tags || [];
  const tagDetails = c.tag_details || {};
  const messages = c.messages || [];
  const dates = c.dates || { created: new Date(c.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) };
        ...
        <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 20, padding: "8px 16px 16px", background: "#fff", borderBottom: "0.5px solid #e5e5ea" }}>
            {[
              ...
              { label: "Edit", icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#007aff" strokeWidth="2" strokeLinecap="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" /></svg> },
            ].map(a => {
              const inner = (
                <>
          ...
          <div style={SHEET_STYLES.section}>
            ...
            <div style={{ padding: "0 16px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
              {dates.created && (
                <div style={SHEET_STYLES.dateRow}><span style={SHEET_STYLES.dateDot} /><span style={SHEET_STYLES.dateLabel}>Created</span><span style={SHEET_STYLES.dateVal}>{dates.created}</span></div>
              )}
              {dates.booked && (
                <div style={SHEET_STYLES.dateRow}><span style={{ ...SHEET_STYLES.dateDot, background: "#007aff" }} /><span style={SHEET_STYLES.dateLabel}>Booked</span><span style={SHEET_STYLES.dateVal}>{dates.booked}</span></div>
              )}
              {dates.appointment && (
                <div style={SHEET_STYLES.dateRow}><span style={{ ...SHEET_STYLES.dateDot, background: "#ff9500" }} /><span style={SHEET_STYLES.dateLabel}>Appointment</span><span style={{ ...SHEET_STYLES.dateVal, color: "#1c1c1e", fontWeight: 600 }}>{dates.appointment}</span></div>
              )}
              {dates.completed && (
                <div style={SHEET_STYLES.dateRow}><span style={{ ...SHEET_STYLES.dateDot, background: "#34a06b" }} /><span style={SHEET_STYLES.dateLabel}>Completed</span><span style={SHEET_STYLES.dateVal}>{dates.completed}</span></div>
              )}
            </div>
          </div>
          ...
          <div style={SHEET_STYLES.section}>
            ...
            <div style={SHEET_STYLES.infoRow}>
              ...
              <div style={SHEET_STYLES.infoVal}>{sourceLabel}</div>
            </div>
              ...
              {tags.length > 0 ? tags.map(t => (
                <div key={t} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "10px 16px", borderTop: "0.5px solid #f2f2f7" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {tagDetails[t] && <div style={{ fontSize: 12, color: "#8e8e93", marginTop: 1 }}>{tagDetails[t]}</div>}
                  </div>
                </div>
              )) : (
          ...
          {messages.length > 0 && (
            <div style={SHEET_STYLES.section}>
              <div style={{ padding: "10px 16px 6px" }}>
                <div style={SHEET_STYLES.secTitle}>Recent Messages</div>
              </div>
              <div style={{ padding: "0 12px 10px", display: "flex", flexDirection: "column", gap: 4 }}>
                {messages.slice(-3).map((m, i) => {
                  const isContact = m.from === "contact";
                  return (
                    <div key={i} style={{ alignSelf: isContact ? "flex-end" : "flex-start", maxWidth: "80%" }}>
                      <div style={{
                        background: isContact ? "#007aff" : "#e9e9eb",
                        color: isContact ? "#fff" : "#1c1c1e",
                        padding: "7px 11px", fontSize: 13, lineHeight: 1.4,
                        borderRadius: isContact ? "14px 14px 4px 14px" : "14px 14px 14px 4px",
                      }}>{m.text}</div>
                      <div style={{ fontSize: 10, color: "#c7c7cc", marginTop: 2, textAlign: isContact ? "right" : "left", padding: "0 4px" }}>
                        {m.from !== "contact" && <span style={{ textTransform: "capitalize" }}>{m.from} &middot; </span>}{m.time}
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ padding: "0 16px 14px" }}>
                <div style={{ fontSize: 13, color: "#007aff", fontWeight: 500, cursor: "pointer" }}>View full conversation</div>
              </div>
            </div>
          )}
